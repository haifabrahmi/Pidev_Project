...
public class AvisUpdateController implements Initializable {
    ...
    String nom = nammodif.getText();
        System.out.println("namoooodif"+nom);
    String commentaire = commenttf.getText();
    int note = Integer.parseInt(notetf.getText());
        System.out.println("getattr"+nom);
    ...
    void setAttribute(Avis a)throws ParseException {
        this.avis = a;
        System.out.println("-----------"+a.getNomUser());
        nammodif.setText(a.getNomUser());
 ...
 */
        System.out.println("eventttttttt"+event); 
    AvisServices as = new AvisServices() {};
    Avis a = getAttribute();
      System.out.println("avant appel"+nammodif.getText());
    a.setIdAvis(avis.getIdAvis()); // Nécessaire pour mettre à jour la réclamation correcte
    a.setNomUser(nammodif.getText());
    as.modifier(a);
    try {
        if (listener != null) {
            listener.onInfoSent(true);
