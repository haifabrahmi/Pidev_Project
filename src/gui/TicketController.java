...
private void imprimer(ActionEvent event) throws DocumentException, FileNotFoundException, IOException, WriterException {
    ...
    String type_ticket = typeticket.getText();
    String prix_totale = price.getText();
    ...
    Font pricefont = new Font(Font.FontFamily.TIMES_ROMAN, 24, Font.BOLD, BaseColor.ORANGE);


    Paragraph datePara = new Paragraph("Date de réservation: " + date_res, fontData);
    datePara.setAlignment(Element.ALIGN_CENTER);
    document.add(datePara);

    Paragraph heurePara = new Paragraph("Heure de réservation: " + heure_res, fontData);
    heurePara.setAlignment(Element.ALIGN_CENTER);
    document.add(heurePara);

    Paragraph nbPlacePara = new Paragraph("Nombre de places réservées: " + nb_place, fontData);
    nbPlacePara.setAlignment(Element.ALIGN_CENTER);
    document.add(nbPlacePara);

    Paragraph typeTicketPara = new Paragraph("Type de ticket: " + type_ticket, fontData);
    typeTicketPara.setAlignment(Element.ALIGN_CENTER);
    document.add(typeTicketPara);

    Paragraph pricePara = new Paragraph("Prix: " + prix_totale + " DT", pricefont);
    ...
    BarcodeQRCode qrCode = new BarcodeQRCode(" date" + date_res + " heure " + heure_res + " nb_place " + nb_place + " type ticket" + type_ticket + " prix" + prix_totale, 100, 100, null);
